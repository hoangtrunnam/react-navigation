...
const TabBarItemInternal = ({
  ...
  const animatedActiveStyle = useAnimatedStyle(() => {
    ...
    const opacity = interpolate(reanimatedPosition.value, inputRange, inputRange.map(i => i === tabIndex ? 1 : 0), Extrapolation.CLAMP);
    return {
      opacity
    };
  ...
  const animatedInactiveStyle = useAnimatedStyle(() => {
    ...
    const opacity = interpolate(reanimatedPosition.value, inputRange, inputRange.map(i => i === tabIndex ? 0 : 1), Extrapolation.CLAMP);
    return {
      opacity
    };
  }, [reanimatedPosition, routesLength, tabIndex]);
  const icon = React.useMemo(() => {
    ...
    return /*#__PURE__*/_jsxs(View, {
      ...
      children: [/*#__PURE__*/_jsx(Animated.View, {
        style: animatedInactiveStyle,
        children: inactiveIcon
      }), /*#__PURE__*/_jsx(Animated.View, {
        style: [StyleSheet.absoluteFill, animatedActiveStyle],
        children: activeIcon
      })]
    });
  }, [activeColor, animatedActiveStyle, animatedInactiveStyle, customIcon, inactiveColor, route]);
  const renderLabel = React.useCallback(focused => customlabel ? customlabel({
    ...
    label: labelText,
    style: labelStyle
  }), [customlabel, labelProps, activeColor, inactiveColor, labelStyle, labelText, labelAllowFontScaling, route, icon]);
